    ...
    if page=="Overview":
        ...
        if has_real_data and "content" in filtered.columns:
            for _,row in filtered.head(4).iterrows():
                st.markdown(f'<div class="voice-card"><div class="voice-text">"{row.get("content","")}"</div><div class="voice-meta">location {row.get("location","-")} <span class="voice-badge">{row.get("channel","-")}</span> Verified</div></div>',unsafe_allow_html=True)
        else:
            for voice,loc,ch,theme in VOICES[:4]:
                st.markdown(f'<div class="voice-card"><div class="voice-text">"{voice}"</div><div class="voice-meta">location {loc}, Kenya <span class="voice-badge">{ch}</span> {theme} Verified</div></div>',unsafe_allow_html=True)
        ...
        d2["Time"]=d2["Timestamp"].dt.strftime("%b %d, %H:%M"); d2["Verified"]=d2["Verified"].map({True:"yes",False:"no"})
        ...
        if st.button("Send report",use_container_width=True):
            if report_email: st.success(f"Report sent to {report_email}")
